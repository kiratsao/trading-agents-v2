...
def _compare_trades(
    ...
    for idx, (nt, ot) in enumerate(zip(new_trades, old_trades), 1):
        ...
        if idx > 30 and first_div is None:
            print(f"  ... (showing first 30; no divergence yet)")
            break
        if idx > 30:
            break

    if new_trades[len(old_trades):]:
        print(f"\n  +{len(new_trades) - len(old_trades)} extra trades in new engine (pyramid or different exits)")
...
def main(argv=None) -> None:
    ...
    p.add_argument("--trade_diff", action="store_true", help="Show trade-by-trade diff vs old engine")
