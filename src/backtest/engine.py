    ...
    def run(
        ...
        for i in range(1, len(df)):
            ...
            if sig.action == "close" and position > 0 and entry_price is not None:
                ...
                # ── Settlement rollover: re-check entry on same bar ──
                if "settlement" in sig.reason:
                    ...
                    if re_sig.action == "buy":
                        n = re_sig.contracts if re_sig.contracts > 0 else 1
                        realized_equity -= COST_PER_SIDE * n
                        position = n
                        entry_price = exec_price
                        entry_date = today
                        entry_reason = f"settlement rollover: {re_sig.reason}"
                        highest_high = exec_price
                        pyramided = False
                        logger.debug(
                            "ROLL  %s px=%.0f n=%d eq=%.0f",
